  ...
  useEffect(() => {
    if (activePage === "users") {
      document
        ...
        ?.setAttribute(
          "style",
          "color: #B3B3B3; font-weight: 700; cursor: pointer"
        );
      document
        .querySelector(".newsTitle")
        ?.classList.remove(styles.activeTitle);
      document.querySelector(".newsTitle")?.classList.add(styles.inactiveTitle);
      document
        .querySelector(".usersTitle")
        ?.setAttribute(
          "style",
          "color: #14397F; font-weight: 700; cursor: pointer"
        );
      document
        .querySelector(".usersTitle")
        ?.classList.remove(styles.inactiveTitle);
      ...
      document
        .querySelector(".usersTitle")
        ?.classList.add(styles.inactiveTitle);
    }
  }, [activePage]);
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex flex-row gap-5">
        <h1
          className={`bg-slate-100 rounded-xl p-2 text-xl font-bold mb-4 newsTitle ${styles.activeTitle}`}
        ...
        <h1
          className={`bg-slate-100 rounded-xl p-2 text-xl font-bold mb-4 usersTitle ${styles.inactiveTitle}`}
          ...
          <div>
            {users.map((user: any) => (
              <div style={{ marginBottom: "4vh" }}>
                <p>
                  <span style={{ fontWeight: "700", color: "#14397F" }}>
                    Nombre:
                  </span>{" "}
                  {user.nombre}
                </p>
          ...
          <DialogHeader>
            ...
            <Button
              onClick={async () => {
                ...
                try {
                  await fetch(
                    "https://buffetec-api.vercel.app/createAttorney",
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
      ...
      {news.map((item) => (
        <Card key={item.id}>
          <CardHeader style={{ paddingBottom: 10 }}>
            <CardTitle style={{ fontSize: 20, color: "#14397F" }}>
              {item.title}
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <p style={{ fontSize: 12, fontWeight: 500 }}>
              {item.description.substring(0, 200).substring(0, 170)}...
            </p>
          </CardContent>
          <CardFooter>
            <Button
